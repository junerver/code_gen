...
import { objectOmit, useDebounceFn, useToggle } from '@vueuse/core';
import {
  genCdnLink,
  genCompilerSfcLink,
  genImportMap,
...
export const useStore = (initial: Initial) => {
  ...
  // 检查 store.files[MAIN_FILE] 是否存在再设置 hidden 属性
  if (store.files[MAIN_FILE]) {
  ...
  watch(
    builtinImportMap,
    newBuiltinImportMap => {
      // 检查 store.files[IMPORT_MAP] 是否存在再合并 import map
      if (store.files[IMPORT_MAP]) {
